...
class ArcusGUI(tk.Tk):
    ...
    def _update_speed_displays(self):
        ...
        for axis in ["x", "y"]:
            ...
            self.axis_vars[axis]["dec"].set(str(params["dec"]))
    ...
    def _build_widgets(self):
        ...
        for idx, axis_label in enumerate([("X", "X"), ("Y", "Y")]):
            ...
            move_var = tk.StringVar(value="14950")
            move_entry = tk.Entry(row_frame, textvariable=move_var, width=8, justify="center", 
                              bg="white", font=("Arial", 9))
            move_entry.place(x=col_widths[0] + col_widths[1]//2, y=25, anchor="center")
            
            # Bouton MOVE TO
            move_btn = tk.Button(row_frame, text="MOVE TO", bg="#d0d0d0", font=("Arial", 8, "bold"),
                             command=lambda a=axis_lower: self.move_axis(a), width=7)
            move_btn.place(x=col_widths[0] + col_widths[1] + col_widths[2]//2, y=25, anchor="center")
            
            # Bouton HOME
            home_btn = tk.Button(row_frame, text="HOME", bg="#d0d0d0", font=("Arial", 8, "bold"),
                              command=lambda a=axis_lower: self.home_axis(a), width=7)
            home_btn.place(x=col_widths[0] + col_widths[1] + col_widths[2] + col_widths[3]//2, y=25, anchor="center")
            ...
            stop_slow_btn.place(x=col_widths[0] + col_widths[1] + col_widths[2] + col_widths[3] + col_widths[4]//2, y=25, anchor="center")
            ...
            stop_now_btn.place(x=col_widths[0] + col_widths[1] + col_widths[2] + col_widths[3] + col_widths[4] + col_widths[5]//2, y=25, anchor="center")
        ...
        tk.Label(headers_frame, text="Low speed", bg="#d5d5d5", font=("Arial", 8, "bold")).grid(row=0, column=1, padx=15)
        tk.Label(headers_frame, text="High speed", bg="#d5d5d5", font=("Arial", 8, "bold")).grid(row=0, column=2, padx=15)
        tk.Label(headers_frame, text="Acceleration", bg="#d5d5d5", font=("Arial", 8, "bold")).grid(row=0, column=3, padx=15)
        tk.Label(headers_frame, text="Deceleration", bg="#d5d5d5", font=("Arial", 8, "bold")).grid(row=0, column=4, padx=15)
        ...
        for idx, axis in enumerate(["x", "y"]):
            ...
            ls_entry.grid(row=idx, column=1, padx=15, pady=5)
            
            # High speed
            hs_entry = tk.Entry(values_frame, textvariable=self.axis_vars[axis]["hs"], width=8, justify="center", bg="white") 
            hs_entry.grid(row=idx, column=2, padx=15, pady=5)
            
            # Acceleration
            acc_entry = tk.Entry(values_frame, textvariable=self.axis_vars[axis]["acc"], width=8, justify="center", bg="white") 
            acc_entry.grid(row=idx, column=3, padx=15, pady=5)
            
            # Deceleration
            dec_entry = tk.Entry(values_frame, textvariable=self.axis_vars[axis]["dec"], width=8, justify="center", bg="white") 
            dec_entry.grid(row=idx, column=4, padx=15, pady=5)
        ...
        set_speeds_btn.grid(row=0, column=5, rowspan=2, padx=10, pady=5)
    ...
    def move_axis(self, axis):
        ...
        try:
            pos = float(self.axis_vars[axis]["move"].get())
            self.controller.move_to(axis, pos)
            self.log_var.set(f"Déplacement axe {axis.upper()} → {pos}")
        except Exception as e:
            self.log_var.set(f"Erreur move {axis}: {e}")
    ...
    def apply_speeds(self):
        ...
        try:
            ...
            for axis in ["x", "y"]:
                ...
                dec = int(self.axis_vars[axis]["dec"].get())
                
                # Appliquer les paramètres via le contrôleur
                self.controller.set_axis_params(axis, ls=ls, hs=hs, acc=acc, dec=dec)
