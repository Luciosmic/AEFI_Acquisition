...
from ArcusPerformaxPythonController.gui.components.ArcusPerformax4EXStage_GeometricalView_Widget import GeometricalViewWidget
from ArcusPerformaxPythonController.gui.components.ArcusPerformax4EXStage_GeometricalParametersConversion_Module import cm_to_inc, inc_to_cm
...
class ArcusGUIModern(QMainWindow):
    ...
    def __init__(self):
        ...
        # Connexion au signal positionChanged
        self.manager.positionChanged.connect(self._on_position_changed)
    ...
    def setup_ui(self):
        ...
        # Onglet visualisation géométrique
        self.visual_widget = GeometricalViewWidget()
        visual_tab = QWidget()
        visual_layout = QVBoxLayout()
        # --- Ajout panneau configuration scan 2D ---
        ...
        # --- Fin ajout panneau configuration scan 2D ---
    ...
    def create_control_panel(self):
        ...
        # Bouton Changer Mode
        self.change_mode_btn = QPushButton("🌀 Changer Mode")
        self.change_mode_btn.setStyleSheet("QPushButton { background-color: #A23B72; color: white; font-weight: bold; padding: 10px; }")
        layout.addWidget(self.change_mode_btn)
    ...
    def setup_connections(self):
        ...
        self.change_mode_btn.clicked.connect(self.toggle_mode)
        
        # Paramètres
        self.speed_params.apply_btn.clicked.connect(self.apply_speed_parameters)
        # Connexion du signal positionChanged pour la visualisation
        self.manager.positionChanged.connect(self._on_position_changed)
        # Connexion du bouton Générer le scan à la mise à jour du sous-rectangle
    ...
    def load_current_parameters(self):
        """Charge les paramètres actuels depuis le manager"""
        try:
            for axis in ["x", "y"]:
                params = self.manager.get_axis_params(axis)
    ...
    def move_axis(self, axis):
        ...
        try:
            ...
            self.manager.move_to(axis, target)
    ...
    def _check_homing_axis_done(self, axis):
        try:
            moving = self.manager.is_moving(axis)
            if not moving:
                ...
                # Mise à jour affichage position
                x_inc = self.manager.get_position('x')
                y_inc = self.manager.get_position('y')
                self._on_position_changed(x_inc, y_inc)
    ...
    def _check_homing_xy_done(self):
        try:
            moving_x = self.manager.is_moving('x')
            moving_y = self.manager.is_moving('y')
    ...
    def update_display(self):
        """Met à jour l'affichage en temps réel (statut, uptime)"""
        try:
    ...
    def _on_position_changed(self, x_inc, y_inc):
        # Conversion inc -> cm pour la visualisation
        from ArcusPerformaxPythonController.gui.components.ArcusPerformax4EXStage_GeometricalParametersConversion_Module import inc_to_cm
        x_cm = inc_to_cm(x_inc)
        y_cm = inc_to_cm(y_inc)
        self.visual_widget.set_current_position(x_cm, y_cm)
        # Mise à jour des widgets de contrôle classiques
        self.x_control.update_position(x_inc)
        self.y_control.update_position(y_inc)
    
    def toggle_mode(self):
        if self.manager.mode == self.manager.MODE_EXPLORATION:
            self.manager.set_mode(self.manager.MODE_EXPORT)
        else:
            self.manager.set_mode(self.manager.MODE_EXPLORATION)
    ...
    def _on_state_changed(self, state):
        ...
        # Le bouton change_mode_btn n'est jamais désactivé
        self.change_mode_btn.setEnabled(True)
    ...
    def _execute_scan2d_from_ui(self):
        ...
        try:
            self.manager.execute_flyscan2d(scan_config)
            self.status_bar.showMessage("✅ Scan 2D terminé")
        except Exception as e:
