...
from infrastructure.mocks.adapter_mock_fly_scan_executor import MockFlyScanExecutor, MockFlyScanExecutorConfig
...
from interface.cli.scan_cli_composite_output_port import ScanCLICompositeOutputPort
...
def main():
    ...
    try:
        with open('/Users/luis/Library/CloudStorage/Dropbox/Luis/1 PROJETS/1 - THESE/Ressources/ExperimentalData_ASSOCE/AEFI_Acquisition/AEFI_Acquisition/.cursor/debug.log', 'a') as f:
            import json
            f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"START","location":"main.py:66","message":"main() function started","data":{},"timestamp":__import__('time').time()*1000}) + '\n')
    ...
    # Quick toggle between mock and real hardware
    USE_MOCK_HARDWARE = True  # Change to False for real hardware
    
    # Generate config from boolean
    adapter_type = "mock" if USE_MOCK_HARDWARE else "real"
    HARDWARE_CONFIG = {
        "motion": adapter_type,
        "acquisition": adapter_type,
        "excitation": adapter_type,
        "continuous": adapter_type,
    }
    ...
    if HARDWARE_CONFIG["motion"] == "real":
        ...
        print("  [motion] -> mock")
        # motion_delay_ms: Fallback delay if domain doesn't calculate duration
        # 50ms is faster than default 100ms for quicker tests
        # In practice, domain calculates duration via set_motion_profile(), so this is rarely used
    ...
    # FlyScan Executor (Infrastructure service - Mock for now)
    # Configure with realistic acquisition latency (MCU communication: 15ms ± 5ms)
    fly_scan_config = MockFlyScanExecutorConfig(
        acquisition_latency_ms=15.0,  # Base MCU communication delay
        acquisition_latency_jitter_ms=5.0,  # Communication variability
        enable_statistics=False  # Set to True for debugging
    )
    fly_scan_executor = MockFlyScanExecutor(config=fly_scan_config, event_bus=event_bus)
    ...
    # Scan Presenter (UI) - created first but output port set later
    scan_presenter = ScanPresenter(scan_service, scan_export_service, event_bus, transformation_service)
    
    # Scan CLI Presenter
    scan_cli_output_port = ScanCLIOutputPort(output_format="json")
    
    # Composite Output Port (CLI → UI)
    scan_composite_output_port = ScanCLICompositeOutputPort(
        cli_output_port=scan_cli_output_port,
        ui_output_port=scan_presenter
    )
    
    # Scan Controller (CLI) - sets composite output port on service
    scan_controller = ScanController(scan_service, scan_composite_output_port)
    scan_controller.initialize(event_bus)
    
    # Ensure service uses composite output port (Controller CLI → Presenter CLI → Presenter UI)
    scan_service.set_output_port(scan_composite_output_port)
    # #region agent log
    try:
        with open('/Users/luis/Library/CloudStorage/Dropbox/Luis/1 PROJETS/1 - THESE/Ressources/ExperimentalData_ASSOCE/AEFI_Acquisition/AEFI_Acquisition/.cursor/debug.log', 'a') as f:
            import json
            f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"A","location":"main.py:311","message":"service output port set","data":{"has_service":bool(scan_service),"has_composite_port":bool(scan_composite_output_port),"service_has_output_port":hasattr(scan_service,'_output_port')},"timestamp":__import__('time').time()*1000}) + '\n')
    ...
    scan_visualization_panel.apply_rotation_toggled.connect(scan_presenter.set_apply_rotation)
    ...
    scan_presenter.scan_data_bulk_update.connect(scan_visualization_panel.on_scan_data_bulk_update)
    print("  [scan] wired")
    
    # FlyScan Panel Wiring
    # Pattern: View UI → Controller CLI → Presenter CLI → Presenter UI → View UI
    flyscan_control_panel = dashboard.panels["flyscan_control"]
    
    # Control -> Controller CLI (via command wrapper)
    def on_flyscan_start_requested_wrapper(params: dict):
        """Convert UI signal to CLI command."""
        # #region agent log
        try:
            with open('/Users/luis/Library/CloudStorage/Dropbox/Luis/1 PROJETS/1 - THESE/Ressources/ExperimentalData_ASSOCE/AEFI_Acquisition/AEFI_Acquisition/.cursor/debug.log', 'a') as f:
                import json
                f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"B","location":"main.py:410","message":"flyscan_start_requested_wrapper called","data":{"params_keys":list(params.keys()) if params else None},"timestamp":__import__('time').time()*1000}) + '\n')
        except (PermissionError, OSError):
            pass  # Ignore log errors
        # #endregion
        # Convert params to command format expected by controller
        command_data = {
            "scan_zone": {
                "x_min": params.get("x_min", 0.0),
                "x_max": params.get("x_max", 10.0),
                "y_min": params.get("y_min", 0.0),
                "y_max": params.get("y_max", 10.0)
            },
            "x_nb_points": params.get("x_nb_points", 3),
            "y_nb_points": params.get("y_nb_points", 3),
            "scan_pattern": params.get("scan_pattern", "SERPENTINE"),
            "motion_profile": params.get("motion_profile", {
                "min_speed": 0.1,
                "target_speed": 10.0,
                "acceleration": 5.0,
                "deceleration": 5.0
            }),
            "desired_acquisition_rate_hz": params.get("desired_acquisition_rate_hz", 100.0),
            "max_spatial_gap_mm": params.get("max_spatial_gap_mm", 0.5),
            "acquisition_rate_hz": params.get("acquisition_rate_hz", params.get("desired_acquisition_rate_hz", 100.0))
        }
        # #region agent log
        try:
            with open('/Users/luis/Library/CloudStorage/Dropbox/Luis/1 PROJETS/1 - THESE/Ressources/ExperimentalData_ASSOCE/AEFI_Acquisition/AEFI_Acquisition/.cursor/debug.log', 'a') as f:
                import json
                f.write(json.dumps({"sessionId":"debug-session","runId":"run1","hypothesisId":"B","location":"main.py:433","message":"calling scan_controller.handle_command","data":{"command":"flyscan_start","has_command_data":bool(command_data)},"timestamp":__import__('time').time()*1000}) + '\n')
        except (PermissionError, OSError):
            pass  # Ignore log errors
        # #endregion
        scan_controller.handle_command("flyscan_start", command_data)
    
    flyscan_control_panel.flyscan_start_requested.connect(on_flyscan_start_requested_wrapper)
    flyscan_control_panel.flyscan_stop_requested.connect(lambda: scan_controller.handle_command("scan_stop", {}))
    flyscan_control_panel.flyscan_pause_requested.connect(lambda: scan_controller.handle_command("scan_pause", {}))
    flyscan_control_panel.flyscan_resume_requested.connect(lambda: scan_controller.handle_command("scan_resume", {}))
    
    # Presenter -> Control (reuse scan signals for FlyScan)
    scan_presenter.status_updated.connect(flyscan_control_panel.update_status)
    scan_presenter.scan_started.connect(lambda scan_id, _: flyscan_control_panel.on_flyscan_started(scan_id))
    scan_presenter.scan_completed.connect(flyscan_control_panel.on_flyscan_completed)
    scan_presenter.scan_failed.connect(flyscan_control_panel.on_flyscan_failed)
    scan_presenter.scan_cancelled.connect(flyscan_control_panel.on_flyscan_cancelled)
    scan_presenter.scan_paused.connect(lambda scan_id, point: flyscan_control_panel.on_flyscan_paused(scan_id, point))
    scan_presenter.scan_resumed.connect(lambda scan_id, point: flyscan_control_panel.on_flyscan_resumed(scan_id, point))
    
    # FlyScan -> Visualization (reuse same visualization)
    scan_presenter.scan_started.connect(on_scan_started_viz)
    scan_presenter.scan_progress.connect(on_scan_progress_viz)
    print("  [flyscan] wired")
