import numpy as np
from PySide6.QtWidgets import QWidget, QVBoxLayout, QComboBox, QLabel, QHBoxLayout, QCheckBox
...
class ScanVisualizationPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # Visualize Rotated Image Checkbox
        self.checkbox_apply_rotation = QCheckBox("Visualize Rotated Image")
        self.checkbox_apply_rotation.setToolTip("Apply sensor rotation to visualization (captured at start of scan)")
        self.checkbox_apply_rotation.toggled.connect(self.apply_rotation_toggled)
        toolbar_layout.addWidget(self.checkbox_apply_rotation)
    ...
    def on_scan_data_bulk_update(self, data: dict):
        """
        Refresh view with new data points.
        Used when toggling between Raw and Rotated views.
        Expects data = {'points': [{'x': float, 'y': float, 'value': dict}, ...]}
        """
        points = data.get('points', [])
        if not points:
            return
            
        # Clear grids? No, we might be updating partial.
        # But for toggle, we want to overwrite.
        # Since we iterate over ALL cached points, we will overwrite.
        
        for p in points:
            self.update_data_point_from_position(p['x'], p['y'], p['value'])
