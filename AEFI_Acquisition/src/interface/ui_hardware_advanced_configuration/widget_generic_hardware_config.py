...
from PyQt6.QtWidgets import (QWidget, QFormLayout, QSpinBox, QDoubleSpinBox, QSlider, 
...
class GenericHardwareConfigTab(QWidget):
    ...
    def __init__(self, title: str, param_specs: List[ParameterSpec], parent=None):
        ...
        # Limit maximum width to prevent excessive stretching
    ...
    def _create_widget(self, spec: ParameterSpec) -> QWidget:
        ...
        if spec.type == 'spinbox':
            # Use QDoubleSpinBox if range or default contains floats, QSpinBox for integers
            has_floats = False
            if spec.range:
                has_floats = isinstance(spec.range[0], float) or isinstance(spec.range[1], float)
            if not has_floats and spec.default is not None:
                has_floats = isinstance(spec.default, float)
...
class AdvancedConfigWidget(QWidget):
    """
    Main widget for the Advanced Configuration Tab.
    
    Contains:
    - Top: Dropdown to select hardware ID.
    - Center: The dynamically generated GenericHardwareConfigTab.
    """
    
    def __init__(self, presenter: 'GenericHardwareConfigPresenter'):
        super().__init__()
        self.presenter = presenter
        self.current_config_widget: Optional[GenericHardwareConfigTab] = None
        ...
        top_layout.addWidget(QLabel("Select Hardware to Configure:"))
        
        self.hw_combo = QComboBox()
        self.hw_combo.currentTextChanged.connect(self._on_hardware_selected)
        top_layout.addWidget(self.hw_combo)
        ...
        # Remove stretch to prevent white space on the right
        
        # Connect Signals
        self.presenter.hardware_list_updated.connect(self._update_combo)
        self.presenter.specs_loaded.connect(self._show_specs)
        self.presenter.status_message.connect(self.status_label.setText)
        
        # Initial fetch
        self.presenter.refresh_hardware_list()
        
    def _update_combo(self, hardware_ids: List[str]):
        self.hw_combo.clear()
        self.hw_combo.addItems(hardware_ids)
        
    def _on_hardware_selected(self, hw_id: str):
        if hw_id:
            self.presenter.select_hardware(hw_id)
            
    def _show_specs(self, hw_id: str, specs: List[ParameterSpec]):
        # Clear previous widget
        if self.current_config_widget:
            self.content_layout.removeWidget(self.current_config_widget)
            self.current_config_widget.deleteLater()
            self.current_config_widget = None
            
        # Create new one
        if not specs:
            self.status_label.setText(f"No tunable parameters for {hw_id}")
            return

        self.current_config_widget = GenericHardwareConfigTab(f"Configuration: {hw_id}", specs)
        self.current_config_widget.config_changed.connect(self.presenter.apply_configuration)
        
        self.content_layout.addWidget(self.current_config_widget)

    # Re-export needed class for type checking if necessary
    # from .presenter_generic_hardware_config import GenericHardwareConfigPresenter
