...
@pytest.fixture
def valid_measurement_uncertainty():
    """Fixture providing valid measurement uncertainty."""
    return MeasurementUncertainty(max_uncertainty_volts=1e-6)

def test_step_scan_config_creation(valid_scan_zone, valid_measurement_uncertainty):
    ...
    config = StepScanConfig(
        ...
        measurement_uncertainty=valid_measurement_uncertainty
    )
    
    assert config.x_nb_points == 10
    assert config.y_nb_points == 20
    assert config.scan_pattern == ScanPattern.SERPENTINE

def test_step_scan_config_immutable(valid_scan_zone, valid_measurement_uncertainty):
...
def test_step_scan_config_rejects_zero_x_points(valid_scan_zone, valid_measurement_uncertainty):
...
def test_step_scan_config_rejects_zero_y_points(valid_scan_zone, valid_measurement_uncertainty):
...
def test_step_scan_config_rejects_negative_stabilization(valid_scan_zone, valid_measurement_uncertainty):
...
def test_step_scan_config_rejects_zero_averaging(valid_scan_zone, valid_measurement_uncertainty):
    """Test that averaging_per_position must be >= 1."""
    with pytest.raises(ValueError, match="averaging_per_position must be >= 1"):
        StepScanConfig(
            ...
            scan_pattern=ScanPattern.SERPENTINE,
            stabilization_delay_ms=100,
            averaging_per_position=0,
            measurement_uncertainty=valid_measurement_uncertainty
        )

def test_step_scan_config_total_points(valid_scan_zone, valid_measurement_uncertainty):
...
def test_step_scan_config_estimated_duration(valid_scan_zone, valid_measurement_uncertainty):
    ...
    config = StepScanConfig(
        ...
        averaging_per_position=5,  # 5 measurements at 1000 Hz = 0.005s
        measurement_uncertainty=valid_measurement_uncertainty
    )
...
def test_step_scan_config_accepts_zero_stabilization(valid_scan_zone, valid_measurement_uncertainty):
