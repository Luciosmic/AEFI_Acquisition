...
## Responsibility
- `MockMotionPort` (`adapter_mock_i_motion_port.py`) : implémenter `IMotionPort` en mémoire. Enregistre l'historique des déplacements, simule la latence dans un thread daemon, et publie les événements `MotionCompleted/Failed/PositionUpdated` sur le bus domaine si disponible.
