...
try:
    ...
    from .components.AD9106_NumericDisplay_Widget import NumericDisplayWidget
    from .components.AD9106_RealtimeGraph_Widget import RealtimeGraphWidget
    from .components.AD9106_AdvancedSettings_Widget import AdvancedSettingsWidget
...
class NumericDisplayWidget(QWidget):
    ...
    def _on_unit_changed(self, unit):
        print(f"[DEBUG NumericDisplay] _on_unit_changed: {unit}")
        self.current_unit = unit
        # 🔄 SYNCHRONISATION EXTERNE: Appelle le callback si défini
        if self.sync_callback:
            print(f"[DEBUG NumericDisplay] Calling sync_callback for unit: {unit}")
            self.sync_callback()
    ...
    def _on_factor_changed(self, factor):
        ...
        # 🔄 SYNCHRONISATION EXTERNE: Appelle le callback si défini
        if self.sync_callback:
            print(f"[DEBUG NumericDisplay] Calling sync_callback for factor: {factor}")
            self.sync_callback()
...
class RealtimeGraphWidget(QWidget):
    ...
    def set_conversion_parameters(self, unit: str, v_to_vm_factor: float):
        ...
        print(f"[DEBUG RealtimeGraph] set_conversion_parameters: unit={unit}, factor={v_to_vm_factor}")
        self.current_unit = unit
        ...
        print(f"[DEBUG RealtimeGraph] Y-axis label mis à jour: 'Valeur ({unit})'")
...
class MainApp(QMainWindow):
    ...
    def _create_main_tab(self) -> QWidget:
        ...
        print(f"[DEBUG MainApp] _create_main_tab() appelée")
        tab = QWidget()
        ...
        print(f"[DEBUG MainApp] Création NumericDisplayWidget...")
        self.display_widget = NumericDisplayWidget(self.acquisition_manager)
        layout.addWidget(self.display_widget)
        print(f"[DEBUG MainApp] NumericDisplayWidget créé: {self.display_widget}")
        
        # Zone droite : Graphique temps réel pyqtgraph
        print(f"[DEBUG MainApp] Création RealtimeGraphWidget...")
        self.graph_widget = RealtimeGraphWidget(self.acquisition_manager)
        layout.addWidget(self.graph_widget)
        print(f"[DEBUG MainApp] RealtimeGraphWidget créé: {self.graph_widget}")
        # Synchronisation des paramètres de conversion entre affichage numérique et graphique
        # Utilisation d'un callback direct pour garantir l'ordre d'exécution
        print(f"[DEBUG MainApp] Appel _setup_widget_synchronization()...")
        self._setup_widget_synchronization()
        print(f"[DEBUG MainApp] _create_main_tab() terminée, retour tab")
        return tab
    ...
    def _setup_widget_synchronization(self):
        """🔄 Configuration de la synchronisation entre widgets NumericDisplay et RealtimeGraph"""
        print(f"[DEBUG MainApp] Configuration synchronisation widgets...")
        print(f"[DEBUG MainApp] display_widget: {self.display_widget}")
        print(f"[DEBUG MainApp] graph_widget: {self.graph_widget}")
        
        # Assignation du callback de synchronisation
        self.display_widget.sync_callback = self._sync_conversion_to_graph
        print(f"[DEBUG MainApp] Callback assigné à display_widget.sync_callback: {self.display_widget.sync_callback}")
        
        # Synchronisation initiale : NumericDisplay → RealtimeGraph
        initial_unit = self.display_widget.get_current_unit()
        initial_factor = self.display_widget.get_v_to_vm_factor()
        print(f"[DEBUG MainApp] Synchronisation initiale: unit={initial_unit}, factor={initial_factor}")
    ...
    def _sync_conversion_to_graph(self):
        ...
        if hasattr(self, 'graph_widget'):
            ...
            print(f"[DEBUG _sync_conversion_to_graph] Synchronisation: unit={unit}, factor={factor}")
            self.graph_widget.set_conversion_parameters(unit, factor)
            print(f"[DEBUG _sync_conversion_to_graph] Graphique synchronisé avec unité: {unit}")
