...
class ADCControlAdvanced(QWidget):
    ...
    def set_timing_config(self, timing_config: dict):
        """Mise à jour externe de la configuration timing (pour synchronisation)"""
        if 'clkin' in timing_config:
            self.clkin_combo.blockSignals(True)
            self.clkin_combo.setCurrentText(str(timing_config['clkin']))
            self.clkin_combo.blockSignals(False)
        if 'iclk' in timing_config:
            self.iclk_combo.blockSignals(True)
            self.iclk_combo.setCurrentText(str(timing_config['iclk']))
            self.iclk_combo.blockSignals(False)
        if 'oversampling' in timing_config:
            self.oversampling_combo.blockSignals(True)
            self.oversampling_combo.setCurrentText(str(timing_config['oversampling']))
            self.oversampling_combo.blockSignals(False)

    def set_reference_config(self, reference_config: dict):
        """Mise à jour externe de la configuration référence (pour synchronisation)"""
        if 'negative_ref' in reference_config:
            self.negative_ref_check.blockSignals(True)
            self.negative_ref_check.setChecked(reference_config['negative_ref'])
            self.negative_ref_check.blockSignals(False)
        if 'high_res' in reference_config:
            self.high_res_check.blockSignals(True)
            self.high_res_check.setChecked(reference_config['high_res'])
            self.high_res_check.blockSignals(False)
        if 'ref_voltage' in reference_config:
            self.ref_voltage_combo.blockSignals(True)
            self.ref_voltage_combo.setCurrentText('4.0V' if reference_config['ref_voltage'] == 1 else '2.442V')
            self.ref_voltage_combo.blockSignals(False)
        if 'ref_selection' in reference_config:
            self.ref_selection_combo.blockSignals(True)
            self.ref_selection_combo.setCurrentText('Internal' if reference_config['ref_selection'] == 1 else 'External')
            self.ref_selection_combo.blockSignals(False)
