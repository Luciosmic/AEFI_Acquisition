    ...
    def __init__(self, port="COM10", serial_communicator=None, adc_converter=None, data_buffer=None, acquisition_sample=None, csv_exporter=None):
        ...
        if serial_communicator is None:
            try:
                ...
                if not success:
                    print(f"[WARNING] Impossible de se connecter au port {port}: {msg}")
                    print("[WARNING] L'application fonctionnera en mode dégradé (sans contrôle DDS/ADC)")
                ...
                print(f"[WARNING] Erreur d'initialisation SerialCommunicator: {e}")
                print("[WARNING] L'application fonctionnera en mode dégradé (sans contrôle DDS/ADC)")
                self._serial_communicator = None
        else:
