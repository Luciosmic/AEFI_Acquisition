...
from .ArcusPerformax4EXStage_DataBuffer_Module import AcquisitionSample
...
@dataclass
class ExportConfigPosition:
    """Configuration d'export CSV pour la position"""
    output_dir: str
    filename_base: str
    metadata: Dict[str, Any]
...
class CSVPositionExporter:
    """
    Exporteur CSV pour les positions (PositionSample)
    """
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        self._export_queue = Queue()
        ...
        self._sample_index = 0
        self._t0 = None
    def start_export(self, config: ExportConfigPosition) -> bool:
        with self._export_lock:
            if self._is_exporting:
                return False
            self._export_config = config
            ...
            self._t0 = time.perf_counter()
            timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
            filename = f"{timestamp}_{config.filename_base}_positions.csv"
            filepath = Path(config.output_dir) / filename
            try:
                filepath.parent.mkdir(parents=True, exist_ok=True)
                self._current_file = open(filepath, 'w', newline='', encoding='utf-8')
    ...
    def _write_metadata(self):
        if not self._csv_writer or not self._export_config:
            return
        self._csv_writer.writerow(['# Configuration Export Position'])
        self._csv_writer.writerow(['# Timestamp:', datetime.now().isoformat()])
        metadata = self._export_config.metadata
        for key, value in metadata.items():
            self._csv_writer.writerow([f'# {key}:', str(value)])
        self._csv_writer.writerow([])
        headers = ['index', 'timestamp', 'x', 'y']
        self._csv_writer.writerow(headers)
    def _write_footer(self):
        if not self._csv_writer:
            return
    ...
    def get_export_status(self) -> Dict[str, Any]:
        return {
            ...
            'samples_written': self._samples_written,
            'queue_size': self._export_queue.qsize(),
            'config': self._export_config.__dict__ if self._export_config else None
        } 