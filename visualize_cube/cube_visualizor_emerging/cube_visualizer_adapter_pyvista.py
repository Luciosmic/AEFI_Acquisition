...
class CubeVisualizerAdapter:
    ...
    def _initialize_plotter(self):
        ...
        try:
            ...
            # Activer le trackball style pour rotation fluide
            self.plotter.enable_trackball_style()
            
            # ACTIVER LE ZOOM AVEC TOUCHPAD/MOLETTE
            # Configurer l'interactor pour supporter le zoom
            if hasattr(self.plotter, 'iren') and self.plotter.iren is not None:
                # Activer le zoom avec la molette/touchpad
                # VTK utilise add_observer (minuscules), pas AddObserver
                self.plotter.iren.add_observer('MouseWheelForwardEvent', self._on_mouse_wheel_forward)
                self.plotter.iren.add_observer('MouseWheelBackwardEvent', self._on_mouse_wheel_backward)
                print("[DEBUG] Zoom touchpad/molette activé")
    ...
    def _on_mouse_wheel_forward(self, obj, event):
        """Callback pour zoom avant (molette/touchpad vers le haut)."""
        try:
            if self.plotter and hasattr(self.plotter, 'camera') and self.plotter.camera:
                self.plotter.camera.zoom(1.1)  # Zoom avant de 10%
                self.plotter.render()
        except Exception as e:
            print(f"[DEBUG] Erreur zoom avant: {e}")
    
    def _on_mouse_wheel_backward(self, obj, event):
        """Callback pour zoom arrière (molette/touchpad vers le bas)."""
        try:
            if self.plotter and hasattr(self.plotter, 'camera') and self.plotter.camera:
                self.plotter.camera.zoom(0.9)  # Zoom arrière de 10%
                self.plotter.render()
        except Exception as e:
            print(f"[DEBUG] Erreur zoom arrière: {e}")
    ...
    def update_view(self, theta_x: Optional[float] = None, theta_y: Optional[float] = None, theta_z: Optional[float] = None):
        ...
        try:
            ...
            # Ajouter les axes _mes (sensor) - TRAVERSENT LE CUBE
            # Les flèches doivent être plus longues que le cube et centrées à l'origine
            cube_size = 1.0
            axis_length = 2.0  # Plus long que le cube
            arrow_radius = 0.03
            
            # Pour que la flèche traverse le cube, on la centre en la démarrant à -axis_length/2
            # et en la terminant à +axis_length/2
            # PyVista Arrow va de start à start+direction*scale
            # On veut: start=-axis_length/2*direction, scale=axis_length
            
            # Axe X du sensor (bleu)
            start_x = (-axis_length/2, 0, 0)
            arrow_x = pv.Arrow(start=start_x, direction=(1, 0, 0), scale=axis_length,
                              tip_radius=arrow_radius, tip_length=0.1, shaft_radius=arrow_radius * 0.6)
            self.arrows_mes['x'] = self.plotter.add_mesh(apply_rotation_to_mesh(arrow_x, rotation), color='#4DA6FF')
            
            # Axe Y du sensor (jaune)
            start_y = (0, -axis_length/2, 0)
            arrow_y = pv.Arrow(start=start_y, direction=(0, 1, 0), scale=axis_length,
                              tip_radius=arrow_radius, tip_length=0.1, shaft_radius=arrow_radius * 0.6)
            self.arrows_mes['y'] = self.plotter.add_mesh(apply_rotation_to_mesh(arrow_y, rotation), color='#FFE633')
            
            # Axe Z du sensor (rouge)
            start_z = (0, 0, -axis_length/2)
            arrow_z = pv.Arrow(start=start_z, direction=(0, 0, 1), scale=axis_length,
                              tip_radius=arrow_radius, tip_length=0.1, shaft_radius=arrow_radius * 0.6)
